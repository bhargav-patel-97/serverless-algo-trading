...
export class PositionExitManager {
  ...
  async monitorAndExecuteExits() {
    ...
    try {
      ...
      if (this.options.enableLogging) {
        this.logger.info('Monitoring positions for exit conditions with persistent storage', {
      ...
      if (this.options.enableLogging) {
        this.logger.info('Position exit monitoring completed with persistent storage', {
          ...exitResults,
          persistentStorageEnabled: this.positionStorage.sheetsLogger?.enabled || false
        });
      }
  ...
  async processPositionForExit(position) {
    ...
    if (this.options.enableLogging) {
      this.logger.info('Processing position for exit conditions with persistent storage', {
        symbol,
        side,
        quantity: absQty,
        currentPrice,
    ...
    try {
      ...
      if (this.options.enableLogging) {
        this.logger.info('Checking exit conditions with persistent storage data', {
          symbol,
          side,
          marketPrice,
      ...
      if (this.shouldTriggerStopLoss(side, marketPrice, storedLevels.stopLoss)) {
        if (this.options.enableLogging) {
          this.logger.warning('Stop loss triggered from persistent storage', {
            symbol,
            side,
            marketPrice,
      ...
      if (this.shouldTriggerTakeProfit(side, marketPrice, storedLevels.takeProfit)) {
        if (this.options.enableLogging) {
          this.logger.info('Take profit triggered from persistent storage', {
            symbol,
            side,
            marketPrice,
      ...
      this.logger.error('Error processing position for exit with persistent storage', {
        symbol,
        error: error.message,
        stack: error.stack
      });
  ...
  async executeExitOrder(position, exitType, storedLevels) {
    ...
    for (let attempt = 1; attempt <= this.options.maxRetries; attempt++) {
      try {
        if (this.options.enableLogging) {
          this.logger.info('Executing exit order with persistent storage cleanup', {
            symbol,
            side,
            quantity: absQty,
            exitType,
        ...
        if (this.options.enableLogging) {
          this.logger.success('Exit order executed successfully with persistent storage cleanup', exitTradeResult);
        }
  ...
  async emergencyStopAllPositions() {
    ...
    try {
      ...
      this.logger.warning('Emergency stop completed with persistent storage cleanup', emergencyResults);
