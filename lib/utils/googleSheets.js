...
// RACE CONDITION FIX: Adds enhanced signal strength querying without self-interference
...
export class GoogleSheetsLogger {
  constructor() {
    ...
    try {
      ...
      this.enabled = true;
      this.logger.info('Google Sheets integration initialized successfully with race condition prevention');
    } catch (error) {
  ...
  async logSignalStrength({ timestamp, symbol, side, strategy, signalStrength, orderId }) {
    ...
    try {
      // RACE CONDITION FIX: Enhanced validation and logging for signal strength
      ...
      if (!validatedSymbol) {
        this.logger.error('RACE CONDITION FIX: Cannot log signal strength - missing symbol', { 
      ...
      this.logger.info('RACE CONDITION FIX: Signal strength logged to Google Sheets AFTER trade execution', {
        symbol: validatedSymbol,
        strategy: validatedStrategy,
        signalStrength: validatedSignalStrength,
        side: validatedSide,
        columnB_symbol: validatedSymbol,
  ...
  async initializeSignalStrengthSheet() {
    ...
    try {
      ...
      this.logger.info('SignalStrength sheet initialized with headers and race condition prevention');
    } catch (error) {
  ...
  // RACE CONDITION FIX: Enhanced getLastSignalStrength with exclusion of recent entries
  async getLastSignalStrength(symbol, side = null, excludeRecentMinutes = 1) {
    if (!this.enabled) {
    ...
    try {
      // RACE CONDITION FIX: Validate input parameters
      const searchSymbol = symbol ? symbol.toString().toUpperCase() : '';
      const searchSide = side ? side.toString().toUpperCase() : null;

      if (!searchSymbol) {
        this.logger.error('RACE CONDITION FIX: Invalid symbol provided', { symbol, side });
        return null;
      }

      this.logger.info('RACE CONDITION FIX: Starting signal strength lookup with recent entry exclusion', {
        searchSymbol,
        searchSide,
      ...
      const rows = response.data.values;
      if (!rows || rows.length <= 1) {
        this.logger.info('RACE CONDITION FIX: No data found in SignalStrength sheet', { 
          searchSymbol,
          searchSide 
        });
        return null;
      ...
      this.logger.info('RACE CONDITION FIX: Found rows to search with recent exclusion', {
      ...
      // RACE CONDITION FIX: Calculate cutoff time to exclude very recent entries
      const cutoffTime = new Date(Date.now() - (excludeRecentMinutes * 60 * 1000));

      // RACE CONDITION FIX: Enhanced search with recent entry exclusion and detailed logging
      let foundRows = [];
      let lastMatchingRow = null;

      // Find all matching rows for debugging, excluding recent entries
      for (let i = rows.length - 1; i > 0; i--) {
        ...
        const rowTimestamp = rows[i][0] ? new Date(rows[i][0]) : null;

        // RACE CONDITION FIX: Skip entries that are too recent (within excludeRecentMinutes)
        if (rowTimestamp && rowTimestamp > cutoffTime) {
          this.logger.info('RACE CONDITION FIX: Skipping recent entry to prevent self-comparison', {
            rowIndex: i,
            rowSymbol,
        ...
        if (i >= rows.length - 10) { // Only log last 10 rows
          this.logger.info('RACE CONDITION FIX: Row debug with recent exclusion', {
            rowIndex: i,
            rowSymbol,
            rowSide,
            rowStrategy,
            ...
            excludedAsRecent: rowTimestamp && rowTimestamp > cutoffTime
          });
        }

        // Check if this row matches our search criteria and is not too recent
      ...
      // RACE CONDITION FIX: Enhanced result reporting with exclusion info
      if (foundRows.length > 0) {
        this.logger.info('RACE CONDITION FIX: Found matching symbol entries (excluding recent)', {
          searchSymbol,
          searchSide,
          totalMatches: foundRows.length,
          excludeRecentMinutes,
          allMatches: foundRows.slice(0, 5).map(r => ({ // Limit to first 5 for logging
            symbol: r.symbol,
            side: r.side,
            strategy: r.strategy,
            strength: r.strength,
      ...
      if (lastMatchingRow && lastMatchingRow.strength !== null && !isNaN(lastMatchingRow.strength)) {
        this.logger.info('RACE CONDITION FIX: Found valid match (excluding recent entries)', {
          searchSymbol,
          searchSide,
          ...
          confirmColumnB: lastMatchingRow.symbol,
          raceConditionPrevented: true
        });
        return lastMatchingRow.strength;
      } else {
        this.logger.info('RACE CONDITION FIX: No valid match found (excluding recent entries)', {
          searchSymbol,
          searchSide,
      ...
      this.logger.error('RACE CONDITION FIX: Failed to get last signal strength from Google Sheets', {
        error: error.message,
        stack: error.stack,
        symbol,
        side,
  ...
  // RACE CONDITION FIX: New method to get signal strength excluding entries with specific order IDs
  async getLastSignalStrengthExcludingOrderIds(symbol, side = null, excludeOrderIds = []) {
    if (!this.enabled) {
      this.logger.warning('Google Sheets disabled - cannot get signal strength', { symbol, side });
      return null;
    }
    ...
    try {
      ...
      this.logger.info('RACE CONDITION FIX: Starting signal strength lookup excluding specific order IDs', {
        searchSymbol,
        searchSide,
        excludeOrderIds
      });
      ...
      // Find the last matching row, excluding specified order IDs
      for (let i = rows.length - 1; i > 0; i--) {
        const rowSymbol = rows[i][1] ? rows[i][1].toString().toUpperCase() : '';
        const rowSide = rows[i][2] ? rows[i][2].toString().toUpperCase() : '';
        const rowStrength = rows[i][4] !== '' ? parseFloat(rows[i][4]) : null;
        const rowOrderId = rows[i][5] ? rows[i][5].toString() : '';

        // Skip if this row has an excluded order ID
        if (excludeOrderIds.includes(rowOrderId)) {
          this.logger.info('RACE CONDITION FIX: Skipping entry with excluded order ID', {
            rowIndex: i,
            rowOrderId,
            excludeOrderIds,
            raceConditionPrevented: true
        ...
        if (rowSymbol === searchSymbol && (!searchSide || rowSide === searchSide)) {
          if (rowStrength !== null && !isNaN(rowStrength)) {
            this.logger.info('RACE CONDITION FIX: Found match excluding order IDs', {
              searchSymbol,
              foundStrength: rowStrength,
              excludeOrderIds,
              raceConditionPrevented: true
            });
            return rowStrength;
      ...
      this.logger.info('RACE CONDITION FIX: No match found excluding order IDs', {
        searchSymbol,
        searchSide,
        excludeOrderIds,
        raceConditionPrevented: true
      ...
      this.logger.error('RACE CONDITION FIX: Error getting signal strength excluding order IDs', {
        error: error.message,
        symbol,
        side,
        excludeOrderIds
  ...
  // =============================================================================
  // CRITICAL FIX: REGIME STATE MANAGEMENT METHODS FOR SERVERLESS
  // =============================================================================
  
  /**
   * CRITICAL FIX: Initialize regime state sheet for tracking market regimes
   */
  async initializeRegimeStateSheet() {
    if (!this.enabled) {
      this.logger.warning('Cannot initialize regime state sheet - integration disabled');
      return;
    }

    try {
      const headers = [
        'Timestamp', 'Base Symbol', 'Regime', 'MA200', 'Current Price', 'Regime Strength', 'Change Date', 'Strategy Name'
      ];

      await this.sheetsAPI.spreadsheets.values.update({
        spreadsheetId: this.spreadsheetId,
        range: 'RegimeState!A1:H1',
        valueInputOption: 'RAW',
        requestBody: { values: [headers] }
      });

      this.logger.info('RegimeState sheet initialized with headers for serverless persistence');
    } catch (error) {
      this.logger.error('Failed to initialize RegimeState sheet', {
        error: error.message,
        stack: error.stack
      });
    }
  ...
  /**
   * CRITICAL FIX: Store regime state for persistence across serverless invocations
   * @param {Object} regimeData - Regime state information
   * @returns {boolean} Success status
   */
  async storeRegimeState(regimeData) {
    if (!this.enabled) {
      this.logger.info('Google Sheets regime state storage skipped - integration disabled');
      return false;
    }

    try {
      // Find existing row for this strategy/base symbol combination
      const existingRowIndex = await this.findRegimeStateRow(regimeData.baseSymbol, regimeData.strategyName);

      const values = [[
        new Date().toISOString(), // timestamp
        regimeData.baseSymbol,
        regimeData.regime, // 'bull' or 'bear'
        parseFloat(regimeData.ma200) || '',
        parseFloat(regimeData.currentPrice) || '',
        parseFloat(regimeData.regimeStrength) || '',
        regimeData.changeDate || new Date().toISOString(),
        regimeData.strategyName || 'RegimeDetection'
      ]];
      ...
      if (existingRowIndex > 0) {
        ...
        await this.sheetsAPI.spreadsheets.values.update({
          spreadsheetId: this.spreadsheetId,
          range: `RegimeState!A${existingRowIndex}:H${existingRowIndex}`,
          valueInputOption: 'RAW',
          requestBody: { values }
        });

        this.logger.info('Regime state updated in Google Sheets', {
          baseSymbol: regimeData.baseSymbol,
          regime: regimeData.regime,
          rowIndex: existingRowIndex,
          strategyName: regimeData.strategyName
        });
      } else {
        // Append new row
        await this.sheetsAPI.spreadsheets.values.append({
          spreadsheetId: this.spreadsheetId,
          range: 'RegimeState!A:H',
          valueInputOption: 'RAW',
          requestBody: { values }
        });

        this.logger.info('Regime state stored in Google Sheets', {
          baseSymbol: regimeData.baseSymbol,
          regime: regimeData.regime,
          strategyName: regimeData.strategyName
        });
      ...
      this.logger.error('Failed to store regime state in Google Sheets', {
        baseSymbol: regimeData.baseSymbol,
        regime: regimeData.regime,
        error: error.message
      });
  ...
  /**
   * CRITICAL FIX: Get last regime state for a strategy/symbol from Google Sheets
   * @param {string} baseSymbol - Base symbol (e.g., 'SPY')
   * @param {string} strategyName - Strategy name
   * @returns {Object|null} Last regime state or null if not found
   */
  async getRegimeState(baseSymbol, strategyName = 'RegimeDetection') {
    if (!this.enabled) {
      this.logger.warning('Google Sheets disabled - cannot get regime state', { baseSymbol, strategyName });
      return null;
    }
    ...
    try {
      ...
      const rows = response.data.values;
      if (!rows || rows.length <= 1) {
        this.logger.info('No regime state found in Google Sheets', { baseSymbol, strategyName });
        return null;
      }

      // Find the row for the strategy/symbol combination (most recent entry)
      for (let i = rows.length - 1; i > 0; i--) {
        const rowBaseSymbol = rows[i][1];
        const rowStrategyName = rows[i][7] || 'RegimeDetection';

        if (rowBaseSymbol === baseSymbol && rowStrategyName === strategyName) {
          const regimeState = {
            timestamp: rows[i][0],
            baseSymbol: rows[i][1],
            regime: rows[i][2], // 'bull' or 'bear'
            ma200: rows[i][3] ? parseFloat(rows[i][3]) : null,
            currentPrice: rows[i][4] ? parseFloat(rows[i][4]) : null,
            regimeStrength: rows[i][5] ? parseFloat(rows[i][5]) : null,
            changeDate: rows[i][6],
            strategyName: rows[i][7] || 'RegimeDetection'
          };

          this.logger.info('Retrieved regime state from Google Sheets', {
            baseSymbol,
            regime: regimeState.regime,
            changeDate: regimeState.changeDate,
            strategyName,
            age: Math.round((Date.now() - new Date(regimeState.timestamp).getTime()) / 1000 / 60) + ' minutes'
          });
      ...
      this.logger.info('No regime state found for strategy/symbol combination', { baseSymbol, strategyName });
      return null;
    } catch (error) {
      this.logger.error('Failed to retrieve regime state from Google Sheets', {
        baseSymbol,
        strategyName,
        error: error.message
      });
  ...
  /**
   * CRITICAL FIX: Find regime state row for a strategy/symbol combination
   * @param {string} baseSymbol - Base symbol
   * @param {string} strategyName - Strategy name
   * @returns {number} Row index (1-based) or -1 if not found
   */
  async findRegimeStateRow(baseSymbol, strategyName = 'RegimeDetection') {
    try {
      const response = await this.sheetsAPI.spreadsheets.values.get({
        spreadsheetId: this.spreadsheetId,
        range: 'RegimeState!B:H'
      });

      const rows = response.data.values;
      if (!rows || rows.length <= 1) return -1;

      // Find the row with matching symbol and strategy (start from end for most recent)
      for (let i = rows.length - 1; i > 0; i--) {
        const rowBaseSymbol = rows[i][0]; // Column B (index 0 in B:H range)
        const rowStrategyName = rows[i][6] || 'RegimeDetection'; // Column H (index 6 in B:H range)
      ...
      this.logger.error('Failed to find regime state row', {
        baseSymbol,
        strategyName,
        error: error.message
      });
  ...
  /**
   * CRITICAL FIX: Get regime history for analysis
   * @param {string} baseSymbol - Base symbol
   * @param {string} strategyName - Strategy name
   * @param {number} daysBack - Days to look back
   * @returns {Array} Array of regime state history
   */
  async getRegimeHistory(baseSymbol, strategyName = 'RegimeDetection', daysBack = 30) {
    if (!this.enabled) {
      return [];
    }
    ...
    try {
      ...
      for (let i = rows.length - 1; i > 0; i--) {
        ...
        if (rowBaseSymbol === baseSymbol && 
            rowStrategyName === strategyName && 
            rowTimestamp && 
            rowTimestamp >= cutoffTime) {
      ...
      this.logger.error('Failed to get regime history from Google Sheets', {
        baseSymbol,
        strategyName,
        daysBack,
        error: error.message
  ...
  // =============================================================================
  // CRITICAL FIX: TRADING STATE MANAGEMENT METHODS FOR SERVERLESS
  // =============================================================================
  ...
  async initializeTradingStateSheet() {
    ...
    try {
      ...
      this.logger.info('TradingState sheet initialized with headers and race condition prevention');
    } catch (error) {
